...
from visualize.pytorch_grad_cam.base_cam import BaseCAM
from visualize.pytorch_grad_cam.utils.find_layers import replace_layer_recursive
from visualize.pytorch_grad_cam.ablation_layer import AblationLayer
...
class AblationCAM(BaseCAM):
    ...
    def get_cam_weights(self,
                        input_tensor_rgb: NestedTensor,
                        input_tensor_t: NestedTensor,
        ...
        with torch.no_grad():
            outputs = self.model(input_tensor_rgb, input_tensor_t)
            handle.remove()
        ...
        print(replace_layer_recursive(self.model, target_layer, ablation_layer))
        ...
        with torch.no_grad():
            ...
            for batch_index, (target, tensor_rgb, mask_rgb, tensor_t, mask_t) in enumerate(zip(targets, input_tensor_rgb.tensors, input_tensor_rgb.mask, input_tensor_t.tensors, input_tensor_t.mask)):
                new_scores = []
                batch_tensor_rgb = tensor_rgb.repeat(self.batch_size, 1, 1, 1)
                batch_tensor_t = tensor_t.repeat(self.batch_size, 1, 1, 1)
                mask_rgb = mask_rgb.repeat(self.batch_size, 1, 1)
                mask_t = mask_t.repeat(self.batch_size, 1, 1)
                ...
                for i in tqdm.tqdm(range(0, number_channels_to_ablate, self.batch_size)):
                    if i + self.batch_size > number_channels_to_ablate:
                        batch_tensor_rgb = batch_tensor_rgb[:(number_channels_to_ablate - i)]
                        batch_tensor_t = batch_tensor_t[:(number_channels_to_ablate - i)]
                        mask_rgb = mask_rgb[:(number_channels_to_ablate - i)]
                        mask_t = mask_t[:(number_channels_to_ablate - i)]
                    ...
                    ablation_layer.set_next_batch(input_batch_index=batch_index,
                                                  ...
                                                  num_channels_to_ablate=batch_tensor_rgb.size(0))


                    score = [target(o).cpu().item() for o in self.model(NestedTensor(batch_tensor_rgb, mask_rgb), NestedTensor(batch_tensor_t, mask_t))]
                    new_scores.extend(score)
                    ablation_layer.indices = ablation_layer.indices[batch_tensor_rgb.size(0):]
