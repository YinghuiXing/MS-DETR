...
class AblationLayer(torch.nn.Module):
    ...
    def activations_to_be_ablated(self, activations, ratio_channels_to_ablate=1.0):
        ...
        # ws: 若ratio_channels_to_ablate=1.0，则所有的通道都需要进行消融实验
    ...
    def set_next_batch(self, input_batch_index, activations, num_channels_to_ablate):
        """ This creates the next batch of activations from the layer.
            ...
            ws: 以batch的形式，同时进行多个通道的ablate操作
        """
        feature = activations[0]
        pos = activations[1][-1]
        src, mask = feature[-1].decompose()
        src = src[input_batch_index, :, :, :].clone().unsqueeze(0).repeat(num_channels_to_ablate, 1, 1, 1)
        pos = pos[input_batch_index, :, :, :].clone().unsqueeze(0).repeat(num_channels_to_ablate, 1, 1, 1)
        mask = mask[input_batch_index, :, :].clone().unsqueeze(0).repeat(num_channels_to_ablate, 1, 1)

        feature = [NestedTensor(src, mask)]

        self.activations = (feature, [pos])

    def __call__(self, x):
        """ ws: 对特征进行ablate操作
            output: [bs, c, h, w],其中某个bs中的某一个通道将被ablate掉
        """
        output = self.activations[0][-1].decompose()[0]
        mask = self.activations[0][-1].decompose()[1]
        pos = self.activations[1][-1]
        ...
        feature = [NestedTensor(output, mask)]

        return feature, [pos]


class AblationLayer4DETREncoderRGB(torch.nn.Module):
    def __init__(self):
        super(AblationLayer4DETREncoderRGB, self).__init__()
    ...
    def set_next_batch(self, input_batch_index, activations, num_channels_to_ablate):
        """ This creates the next batch of activations from the layer.
            ...
            ws: 以batch的形式，同时进行多个通道的ablate操作
        """
        activations['output_rgb'] = activations['output_rgb'][:, input_batch_index, :].unsqueeze(1).repeat(1, num_channels_to_ablate, 1)
        activations['output_t'] = activations['output_t'][:, input_batch_index, :].unsqueeze(1).repeat(1, num_channels_to_ablate, 1)
        activations['bottlenecks'] = activations['bottlenecks'][:, input_batch_index, :].unsqueeze(1).repeat(1, num_channels_to_ablate, 1)
        activations['bottleneckPaddingMask'] = activations['bottleneckPaddingMask'][input_batch_index].unsqueeze(0).repeat(num_channels_to_ablate, 1)
        activations['bottleneckPos'] = activations['bottleneckPos'][:, input_batch_index, :].unsqueeze(1).repeat(1, num_channels_to_ablate, 1)

        self.activations = activations

    def __call__(self, src_rgb,
                src_t,
                src_key_padding_mask_rgb: Optional[Tensor] = None,
                src_key_padding_mask_t: Optional[Tensor] = None,
                pos_rgb: Optional[Tensor] = None,
        ...
        output_rgb = output['output_rgb']
        for i in range(output_rgb.size(1)):
            # Commonly the minimum activation will be 0,
            # And then it makes sense to zero it out.
            # However depending on the architecture,
            # If the values can be negative, we use very negative values
            # to perform the ablation, deviating from the paper.
            if torch.min(output_rgb) == 0:
                output_rgb[:, i, self.indices[i]] = 0
            else:
                ABLATION_VALUE = 1e7
                output_rgb[:, i, self.indices[i]] = torch.min(
                    output_rgb) - ABLATION_VALUE

        output['output_rgb'] = output_rgb

        return output

class AblationLayer4DETREncoderT(torch.nn.Module):
    def __init__(self):
        super(AblationLayer4DETREncoderT, self).__init__()
    ...
    def __call__(self, src_rgb,
        ...
        for i in range(output_t.size(1)):
            # Commonly the minimum activation will be 0,
            # And then it makes sense to zero it out.
            # However depending on the architecture,
            # If the values can be negative, we use very negative values
            # to perform the ablation, deviating from the paper.
            if torch.min(output_t) == 0:
                output_t[:, i, self.indices[i]] = 0
            else:
                ABLATION_VALUE = 1e7
                output_t[:, i, self.indices[i]] = torch.min(
                    output_t) - ABLATION_VALUE
