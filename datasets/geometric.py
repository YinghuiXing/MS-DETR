...
from .box_type import autocast_box_type
...
class GeomTransform(BaseTransform):
    ...
    def _transform_bboxes(self, results: dict, mag: float) -> None:
        ...
        for key in results['bbox_fields']:
            results[key].project_(self.homography_matrix)
            results[key].clip_(results['img_shape'])
    ...
    def transform(self, results: dict) -> dict:
        ...
        # print(self.__class__.__name__, ':', mag)

        self.homography_matrix = self._get_homography_matrix(results, mag)
        self._record_homography_matrix(results)
        self._transform_img(results, mag)
        if results.get('bbox_fields', None) is not None and len(results.get('bbox_fields', None)):
