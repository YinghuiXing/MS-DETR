...
register_box('hbox', HorizontalBoxes)
    ...
    def decorator(func: Callable) -> Callable:
        def wrapper(self, results: dict, *args, **kwargs) -> dict:
            if ('bbox_fields' not in results
                    or not len(results['bbox_fields'])
                    or isinstance(results[results['bbox_fields'][0]], BaseBoxes)):
                # print(results[results['bbox_fields'][0]])
                # print(results[results['bbox_fields'][1]])
                return func(self, results)
            elif isinstance(results[results['bbox_fields'][0]], np.ndarray):
                for key in results['bbox_fields']:
                    results[key] = box_type_cls(
                        results[key], clone=False)

                # if 'mix_results' in results:
                #     # print('111yes')
                #     for res in results['mix_results']:
                #         if isinstance(res[res['bbox_fields'][0]], np.ndarray):
                #             for key_inner in res['bbox_fields']:
                #                 res[key_inner] = box_type_cls(
                #                     res[key_inner], clone=False)

                # print('11yes')
                # print('rgb', type(results['gt_bboxes_rgb']))
                # print('t', type(results['gt_bboxes_t']))
                ...
                # back to np.ndarray
                for key in _results['bbox_fields']:
                    if isinstance(_results, dict) and key in _results:
                        if isinstance(_results[key], BaseBoxes):
                            _results[key] = _results[key].numpy()
                for key in results['bbox_fields']:
