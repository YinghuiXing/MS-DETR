...
class RCAB(nn.Module):
    def __init__(self, num_channels, reduction, res_scale, use_kernal_3=True):
        super().__init__()
...
class Group(nn.Module):
    def __init__(self, num_channels, num_blocks, reduction, res_scale=1.0, use_kernal_3=True):
        super().__init__()

        body = list()
        for _ in range(num_blocks):
            body += [RCAB(num_channels, reduction, res_scale, use_kernal_3)]
        if use_kernal_3:
            body += [nn.Conv1d(num_channels, num_channels, 3, 1, 1)]
        else:
            body += [nn.Conv1d(num_channels, num_channels, 1, 1, 0)]
        self.body = nn.Sequential(*body)
